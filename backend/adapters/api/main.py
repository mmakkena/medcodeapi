...
from adapters.api.middleware.rate_limit import init_redis, close_redis
...
app = FastAPI(
    ...
    openapi_tags=[
        ...
        {
            "name": "CDI Analysis",
            "description": "Clinical Documentation Integrity analysis: entity extraction, documentation gaps, and CDI query generation"
        },
        {
            "name": "HEDIS Evaluation",
            "description": "Healthcare Effectiveness Data and Information Set (HEDIS) quality measure evaluation"
        },
        {
            "name": "Revenue Optimization",
            "description": "Revenue analysis including E/M coding, HCC opportunities, and DRG optimization"
        }
    ],
    contact={
...
async def health():
    ...
    from adapters.api.middleware.rate_limit import redis_client
...
from adapters.api.routes import auth, icd10, cpt, procedure, suggest, api_keys, usage, billing, clinical_coding, admin, fee_schedule
from adapters.api.routes import cdi, hedis, revenue
...
app.include_router(hedis.router, prefix="/api/v1/hedis", tags=["HEDIS Evaluation"])
